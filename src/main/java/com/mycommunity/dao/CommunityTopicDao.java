...
public interface CommunityTopicDao {
    ...
    //获得普通帖数据 分页
    List<CommunityTopic> getGeneralPartition_ByForumId(@Param("currentPage")int currentPage,@Param("row")int row,@Param("forumId") int forumId,@Param("title")String title);

    //获得记录总行数
    int rowsCount(@Param("forumId") int forumId,@Param("title")String title);
}
